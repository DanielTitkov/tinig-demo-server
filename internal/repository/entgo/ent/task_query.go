...
func (tq *TaskQuery) QueryType() *TaskTypeQuery {
	query := &TaskTypeQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) sqlAll(ctx context.Context) ([]*Task, error) {
	...
	if tq.withUser != nil || tq.withType != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withType; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Task)
		for i := range nodes {
			if fk := nodes[i].task_type_tasks; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(tasktype.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "task_type_tasks" returned %v`, n.ID)
			}
			for i := range nodes {
